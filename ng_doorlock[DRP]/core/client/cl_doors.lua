...
function jobCheck()
    ...
    if exports['drp_id']:SpawnedInAndLoaded() then
        timeout = 10000
        DRP.NetCallbacks.Trigger('ng_doorlock_sv:checkJob', function(job)
            Data.Player.Job = job
        end)
    end
    SetTimeout(timeout, jobCheck)
end
jobCheck()
...
-- Event Handlers --
---------------------------
-- //Syncing Doors on Spawn// --
AddEventHandler('playerSpawned', function()
    if not Data.Player.Loaded then
